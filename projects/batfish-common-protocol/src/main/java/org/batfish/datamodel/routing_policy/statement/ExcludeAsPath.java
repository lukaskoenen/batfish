...
@ParametersAreNonnullByDefault public final class ExcludeAsPath extends Statement {
  private static final String PROP_EXPR = "expr";

  @Nonnull private AsPathListExpr _expr;

  @JsonCreator private static ExcludeAsPath jsonCreator(
      @Nullable @JsonProperty(PROP_EXPR) AsPathListExpr expr) {
  ...
  @Override public <T, U> T accept(StatementVisitor<T, U> visitor, U arg) {
  ...
  @Override public Result execute(Environment environment) {
    if ((environment.getOutputRoute() instanceof HasWritableAsPath<?, ?>)) {
      List<AsSet> excludeAsPath = _expr.evaluate(environment)
          .stream()
          .map(AsSet::of)
      ...
      if (Collections.indexOfSubList(outputRoute.getAsPath().getAsSets(), excludeAsPath) != -1) {
        outputRoute.setAsPath(AsPath.of(ImmutableList.<AsSet>builder()
            .addAll(outputRoute.getAsPath()
                .getAsSets()
                .stream()
      ...
      if (environment.getWriteToIntermediateBgpAttributes()) {
        ...
        if (Collections.indexOfSubList(ir.getAsPath().getAsSets(), excludeAsPath) != -1) {
          ir.setAsPath(AsPath.of(ImmutableList.<AsSet>builder()
              .addAll(outputRoute.getAsPath()
                  .getAsSets()
                  .stream()
                  .filter(currentAsSet -> !excludeAsPath.contains(currentAsSet))
  ...
  @JsonProperty(PROP_EXPR) @Nonnull public AsPathListExpr getExpr() {
    return _expr;
  }
